...
class PreflightTests(FlightCase):
    def checks(self, plan: dict, *, roster: Roster | None = None) -> list[preflight.Check]:
        flight = self.seed(plan, approve=False)
        return preflight.run(flight, read_plan(flight.plan_path), roster or self.roster, environment=self.env)
    ...
    def test_everything_passes_with_the_fake_roster(self) -> None:
        ...
        self.assertEqual(kinds, ["cli", "command", "dependency", "staffing"])
        self.assertFalse((self.root / ".autopilot" / "runtime" / "logs").exists(), "preflight launches no paid agent")

    def test_unknown_role_fails(self) -> None:
        plan = toy_plan([task(1, "a", role="desiner")])
        checks = self.checks(plan)
        failed = [check for check in checks if not check.ok]
        self.assertEqual([(check.kind, check.subject) for check in failed], [("staffing", "desiner")])
    ...
    def test_missing_cli_and_failing_command(self) -> None:
        broken = write_roster(self.base / "broken.toml", cli="/nonexistent/agent")
        roster = Roster(environment=dict(self.env, DELEGATE_ROSTER=str(broken)))
    ...
    def test_start_refuses_an_unapproved_plan(self) -> None:
        flight = self.seed(toy_plan([task(1, "a", role="desiner")]), approve=False)
        started = self.cli("start")
        self.assertEqual(started.returncode, 1)
        self.assertIn("not approved", started.stderr)
        flight.load()
        self.assertEqual(flight.data["status"], "planned")
        self.assertEqual(self.cli("preflight").returncode, 1)

    def test_start_passes_preflight_and_reports_it(self) -> None:
        self.seed(toy_plan([task(1, "a")]))
        started = self.cli("start")
        self.assertEqual(started.returncode, 0, started.stderr)
        self.assertIn("ok   staffing planner", started.stdout)
